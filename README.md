# SmolVLA + LoRA Grasping Fine-Tune — hyzhang01/GCA_parallel_adaptation_b01
...
python 03_evaluate.py --checkpoint outputs/smolvla_lora_grasp/checkpoints/last
```
...
1. **`01_prepare_data.py`** assumes `dataset.meta.tasks` / `dataset.meta.episodes[i]["length"]`
   ...
   `python -c "from lerobot.datasets.lerobot_dataset import LeRobotDataset; d=LeRobotDataset('hyzhang01/GCA_parallel_adaptation_b01'); print(d.meta)"`
...
4. Neither script here has been executed against live GPU + the actual
   ...
   first run.
5. for running the client
   python client/Smolvla_Client.py   --host 127.0.0.1   --port 6002   --task Grasp-Franka-IK-Rel-img   --task_id b01   --num_demos 1# msc-project
