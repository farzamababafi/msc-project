...
# Must match dataset.meta.tasks for hyzhang01/GCA_suction_franka_a01_id0.
DATASET_TASK = "pick up the cracker box"
...
def _to_hwc_rgb_uint8(image: np.ndarray) -> np.ndarray:
    """Isaac cameras may be HWC/CHW, RGB/RGBA, float[0,1] or uint8. Dataset is HWC RGB uint8."""
    image = np.asarray(image)
    if image.ndim == 3 and image.shape[0] in (3, 4) and image.shape[-1] not in (3, 4):
        image = np.transpose(image, (1, 2, 0))
    if image.ndim != 3 or image.shape[-1] not in (3, 4):
        raise ValueError(f"Expected HWC RGB(A) image, got shape {image.shape}")
    image = image[..., :3]
    if image.dtype != np.uint8:
        max_val = float(np.nanmax(image)) if image.size else 0.0
        if max_val <= 1.0:
...
def _hwc_to_chw_float(image: np.ndarray) -> torch.Tensor:
    image = _to_hwc_rgb_uint8(image)
    # Copy: base64/frombuffer arrays are often read-only; torch.from_numpy warns otherwise.
    tensor = torch.from_numpy(image.copy()).to(dtype=torch.float32) / 255.0
    return tensor.permute(2, 0, 1).contiguous()


def _resolve_task(task: str) -> str:
    """Refuse Isaac env ids / empty prompts; SmolVLA was trained on one dataset string."""
    task = (task or "").strip()
    if not task or task.startswith("Grasp-") or task == DATASET_TASK:
        if task != DATASET_TASK:
            logger.warning("Replacing client task %r with dataset instruction %r", task, DATASET_TASK)
        return DATASET_TASK
    return task
...
class SmolVLAService:
    ...
    def predict(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        task = _resolve_task(payload.get("task") or payload.get("instruction") or "")

        if not self._logged_first_obs:
            self._logged_first_obs = True
            logger.info(
                "First infer: task=%r image=%s wrist=%s state=%s",
                task,
                tuple(np.asarray(image).shape),
                tuple(np.asarray(wrist_image).shape),
                np.array2string(state, precision=3),
            )
...
def main() -> None:
    ...
    parser.add_argument(
        "--model_path",
        type=str,
        default="/home/fyp/code/Farzam/files/outputs/smolvla_lora_suction_a01/checkpoints/last",
    )
