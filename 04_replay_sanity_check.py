...
def main():
    ...
    parser.add_argument("--frame_offset", type=int, default=30, help="Frame within that episode to start at")
    ...
    # Filtered to one episode: local indices are 0 .. length-1 (lerobot >=0.6
    # no longer exposes episode_data_index on LeRobotDataset).
    ep_len = int(dataset.meta.episodes[args.episode]["length"])
    idx = args.frame_offset

    if idx + args.compare_steps >= ep_len:
        raise ValueError(
            f"episode {args.episode} too short for frame_offset={args.frame_offset} "
            f"+ compare_steps={args.compare_steps}. Episode has {ep_len} frames."
        )
